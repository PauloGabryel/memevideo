...
export async function getConnectionStatus(
  ...
  return (activeRows.find(row => row.status === 'accepted')
    || activeRows.find(row => row.requester_id === userId && row.addressee_id === targetId && row.status === 'pending')
...
export async function sendConnectionRequest(
  ...
  const sameDirectionRejectedRow = rows.find(
    row => row.status === 'rejected'
      && row.requester_id === requesterId
      && row.addressee_id === addresseeId
  );
  if (sameDirectionRejectedRow) {
    const { data, error } = await withTimeout(() =>
      supabase
        .from('connections')
        .update({ status: 'pending', updated_at: new Date().toISOString() })
        .eq('id', sameDirectionRejectedRow.id)
        .select('*')
        .single()
    , REQUEST_TIMEOUT_MS);
