...
export default function ProfilePage() {
  ...
  const loadProfileData = async () => {
    ...
    try {
      ...
      const [postsData, statsData] = await Promise.all([
        fetchUserPosts(profileData.id, user?.id),
  ...
  const handleConnect = async () => {
    ...
    if (!currentUser || !currentProfile) {
      ...
      if (!data?.session?.user) {
        toast.error('Faça login para gerenciar conexões.');
        navigate('/login');
        return;
      }
    ...
    setConnecting(true);
    try {
      const latestConnection = await getConnectionStatus(currentUser.id, currentProfile.id);
      if (latestConnection) {
        await removeConnectionByUsers(currentUser.id, currentProfile.id);
        setConnection(null);
        toast.success('Conexão removida.');
      } else {
        const conn = await sendConnectionRequest(currentUser.id, currentProfile.id);
        setConnection(conn);
        toast.success(conn.status === 'accepted' ? 'Conexão aceita.' : 'Solicitação enviada.');
      }

      const refreshedConnection = await getConnectionStatus(currentUser.id, currentProfile.id);
      setConnection(refreshedConnection);
      await loadProfileData();
    } catch (err) {
      const isConflict = (err as any)?.status === 409 || (err as any)?.code === '23505' || (err as any)?.message?.toLowerCase().includes('conflict') || (err as any)?.message?.toLowerCase().includes('duplicate');
      ...
      // se o erro for de autenticação/refresh token, desloga e redireciona
      if (isAuthError(err)) {
        try {
          await signOut();
        } catch {}
  ...
  const connectionIsIncoming = connection?.status === 'pending' && connection?.addressee_id === user?.id;
  const connectionIsOutgoing = connection?.status === 'pending' && connection?.requester_id === user?.id;
              ...
              {isOwnProfile ? (
                ...
                <>
                  ...
                  <Button
                    ...
                    disabled={connecting || connectionIsIncoming}
                    className="gap-1.5"
                  >
                    {connecting ? (
